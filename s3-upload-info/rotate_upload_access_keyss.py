...
def update_local_creds(profile_name: str, 
    ...
    try:
        ...
        if config.get(profile_name, 'aws_access_key_id')==aws_access_key_id:
            sys.exit(print('\nOK! Local credentials for the', f'"{profile_name}"', 'profile are the latest currently available.\n'))
        ...
        sys.exit(print(f'\n', f'ERROR! Local credentials for the "{profile_name}" profile may be expired.', default_contact_message, '\n'))
    else:
        print('\nOK! Local credentials for the', f'"{profile_name}"', 'profile were updated successfully.\n')
...
def get_active_creds(profile_name: str, region_name: str="us-east-1"):
    try:
        ...
        sys.exit(print(f'\n', f'ERROR! Local credentials are not stored for the "{profile_name}" profile.', default_contact_message, '\n'))
    except ClientError as e:
        sys.exit(print(f'\n', f'ERROR! Local credentials for the "{profile_name}" profile are expired or otherwise invalid.', default_contact_message, '\n'))
    else:
